...
template <class T>
static vector3<T> parse_vec3(std::string const& s) {
  std::vector<std::string> ds;
  tokenize(s, ",", ds);
  vector3<T> val(T(0),T(0),T(0));
  val.x() = string_to_type<T>(trim(ds[0]));
  val.y() = string_to_type<T>(trim(ds[1]));
  val.z() = string_to_type<T>(trim(ds[2]));
  return val;
}
...
void parse_input(
  ...
  while (std::getline(file, line)) {
    ...
    key = trim(key);
    val = trim(val);
    if      (key == "p") in.p = string_to_type<int>(val);
    else if (key == "tensor") in.tensor = string_to_type<bool>(val);
    ...
    else if (key == "gamma") in.gamma = string_to_type<double>(val);
    else if (key == "tfinal") in.tfinal = string_to_type<double>(val);
    else if (key == "CFL") in.CFL = string_to_type<double>(val);
    else if (key == "beta") in.beta = string_to_type<double>(val);
    else if (key == "M") in.M = string_to_type<double>(val);
    else if (key == "gravity") in.gravity = string_to_type<double>(val);
    else if (key == "gravity_axis") in.gravity_axis = string_to_type<int>(val);
    else if (key == "step_frequency") in.step_frequency = string_to_type<int>(val);
    else if (key == "out_frequency") in.out_frequency = string_to_type<double>(val);
    else if (key == "amr_frequency") in.amr_frequency = string_to_type<double>(val);
    else if (key == "error_regression") in.error_regression = string_to_type<double>(val);
    else {
...
void verify_input(Input const& in) {
  if ((in.p < 0) || (in.p > max_p)) throw std::runtime_error("input - invalid p");
...
static vtk::VizView<double> get_density(Block const& block, int soln_idx) {
  CALI_CXX_MARK_FUNCTION;
  Basis const b = block.basis();
  grid3 const g = block.cell_grid();
  grid3 const cell_grid = generalize(g);
  grid3 const viz_cell_grid = generalize(get_viz_cell_grid(g, b.p));
  grid3 const inner_grid = tensor_bounds(b.dim, b.p);
  View<double***> U = block.soln(soln_idx);
  vtk::VizView<double> rho;
  Kokkos::resize(rho, viz_cell_grid.size(), 1);
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) {
    int const cell = cell_grid.index(cell_ijk);
    for_each(execution::hot, inner_grid,
    [&] (vector3<int> const& inner_ijk) P3A_ALWAYS_INLINE {
      int const pt = inner_grid.index(inner_ijk);
      vector3<int> const viz_cell_ijk = (b.p+1)*cell_ijk + inner_ijk;
...
static vtk::VizView<double> get_velocity(Block const& block, int soln_idx) {
  CALI_CXX_MARK_FUNCTION;
  Basis const b = block.basis();
  grid3 const g = block.cell_grid();
  grid3 const cell_grid = generalize(g);
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) {
    ...
    [&] (vector3<int> const& inner_ijk) P3A_ALWAYS_INLINE {
      ...
      vector3<double> const m = interp_vec3_viz(U, b, cell, pt, MM);
      vector3<double> const val = m/rho;
...
static vtk::VizView<double> get_pressure(
    Block const& block,
    int soln_idx,
    double gamma) {
  CALI_CXX_MARK_FUNCTION;
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) {
    ...
    [&] (vector3<int> const& inner_ijk) P3A_ALWAYS_INLINE {
      ...
      vector3<int> const viz_cell_ijk = (b.p+1)*cell_ijk + inner_ijk;
      int const viz_cell = viz_cell_grid.index(viz_cell_ijk);
      static_vector<double, NEQ> const U_pt = interp_vec_viz<NEQ>(U, b, cell, pt);
...
void write_mesh(
  ...
  if (mesh.comm()->rank() == 0) {
    ...
    vtk::write_tree(tree_path, mesh);
    vtk::write_pvtu_start(stream, mesh.leaves().size());
    vtk::write_pvtu_cell_field<double>(stream, "density", 1);
    vtk::write_pvtu_cell_field<double>(stream, "velocity", DIMS);
    vtk::write_pvtu_cell_field<double>(stream, "pressure", 1);
    vtk::write_pvtu_end(stream);
    write_stream(pvtu_path, stream);
  }
  for (Node* leaf : mesh.owned_leaves()) {
    ...
    vtk::write_vtu_start(stream, block);
    vtk::write_vtu_cell_field(stream, "density", get_density(block, soln_idx));
    vtk::write_vtu_cell_field(stream, "velocity", get_velocity(block, soln_idx));
    vtk::write_vtu_cell_field(stream, "pressure", get_pressure(block, soln_idx, in.gamma));
    vtk::write_vtu_end(stream);
    write_stream(block_path, stream);
  }
}
