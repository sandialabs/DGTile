...
static void handle_error_code(
  ...
  copy(execution::par, state.error_code.begin(), state.error_code.end(), &host_error);
  if (host_error == 1) {
...
double compute_stable_time_step(State& state, Block const& block) {
  ...
  simd_view<double***> U = block.simd_soln(0);
  volatile std::int8_t* error_ptr = state.error_code.begin();
  auto f = [=] P3A_DEVICE (
      vector3<int> const& cell_ijk,
      device_simd_mask<double> const& mask) {
    int const cell = cell_grid.index(cell_ijk);
    static_vector<device_simd<double>, NEQ> const U_avg = gather_avg<NEQ>(U, cell, mask);
    vector3<device_simd<double>> const v_avg = get_vec3(U_avg, MM) / U_avg[RH];
    device_simd<double> const c = get_wave_speed(U_avg, gamma);
    if (any_of((c != c) && mask)) { *error_ptr = 2; }
    device_simd<double> dvdx = 0.;
    for (int axis = 0; axis < dim; ++axis) {
      dvdx += (abs(v_avg[axis]) + c) / dx[axis];
    }
    device_simd<double> const dt = 1./(factor*dvdx);
    return dt;
  };
  double constexpr identity_value = maximum_value<double>();
  auto constexpr binary_op = minimizer<double>();
  double const result = simd_transform_reduce(
...
void compute_intr_fluxes(
  ...
  grid3 const g = block.cell_grid();
  grid3 const cell_grid = generalize(g);
  grid3 const side_grid = generalize(get_side_grid(g, axis));
  subgrid3 const intr_sides = generalize(get_intr_sides(g, axis));
  Basis const b = block.basis();
  int const nside_pts = num_pts(dim-1, b.p);
  double const gamma = state.in.gamma;
  simd_view<double***> soln = block.simd_soln(soln_idx);
  simd_view<double***> fluxes = block.simd_flux(axis);
  volatile std::int8_t* error_ptr = state.error_code.begin();
  auto f = [=] P3A_DEVICE (
      vector3<int> const& side_ijk,
      device_simd_mask<double> const& mask) {
    device_simd<double> P[ndirs], c[ndirs];
    static_vector<device_simd<double>, NEQ> U[ndirs], F[ndirs], F_hllc;
    int const side = side_grid.index(side_ijk);
    for (int pt = 0; pt < nside_pts; ++pt) {
      for (int lr = 0; lr < ndirs; ++lr) {
        int const ilr = invert_dir(lr);
        vector3<int> const cell_ijk = get_sides_adj_cell(side_ijk, axis, lr);
        int const cell = cell_grid.index(cell_ijk);
        U[lr] = interp_vec_side<NEQ>(soln, b, cell, axis, ilr, pt, mask);
  ...
  simd_for_each<double>(execution::par, intr_sides, f);
  handle_error_code(state, "compute_intr_fluxes");
}

[[nodiscard]] P3A_ALWAYS_INLINE P3A_HOST_DEVICE inline
static_vector<double, NEQ> gather_side(View<double***> soln, int bside, int pt) {
  static_vector<double, NEQ> U;
  for (int eq = 0; eq < NEQ; ++eq) {
...
void compute_border_fluxes(
  ...
  grid3 const g = block.cell_grid();
  grid3 const side_grid = generalize(get_side_grid(g, axis));
  subgrid3 const border_sides = generalize(get_adj_sides(g, axis, dir));
  grid3 const bside_grid(border_sides.extents());
  Basis const b = block.basis();
  int const nside_pts = num_pts(dim-1, b.p);
  double const gamma = state.in.gamma;
  Border const& border = block.border(axis, dir);
  static_array<View<double***>, ndirs> bsoln = border.soln();
  View<double***> fluxes = block.flux(axis);
  volatile std::int8_t* error_ptr = state.error_code.begin();
  auto f = [=] P3A_DEVICE (vector3<int> const& side_ijk) {
    double P[ndirs], c[ndirs];
    static_vector<double, NEQ> U[ndirs], F[ndirs], F_hllc;
  ...
  for_each(execution::par, border_sides, f);
  handle_error_code(state, "compute_border_fluxes");
}

[[nodiscard]] P3A_ALWAYS_INLINE P3A_HOST_DEVICE inline
static_vector<double, NEQ> gather_amr_side(
...
void compute_amr_border_fluxes(
  ...
  Basis const b = block.basis();
  int const nside_pts = num_pts(dim-1, b.p);
  int const nchild_sides = num_child(dim-1);
  double const gamma = state.in.gamma;
  Border const& border = block.border(axis, dir);
  static_array<View<double****>, ndirs> bsoln = border.amr_soln();
...
void compute_vol_integral(State& state, Block& block, int soln_idx) {
  ...
  Basis const b = block.basis();
  double const gamma = state.in.gamma;
  int const nintr_pts = num_pts(dim, b.p);
  simd_view<double***> R = block.simd_resid();
  simd_view<double***> soln = block.simd_soln(soln_idx);
  volatile std::int8_t* error_ptr = state.error_code.begin();
  auto f = [=] P3A_DEVICE (
      vector3<int> const& cell_ijk,
      device_simd_mask<double> const& mask) {
    device_simd<double> P, val;
    static_vector<device_simd<double>, NEQ> U, F;
    int const cell = cell_grid.index(cell_ijk);
    for (int pt = 0; pt < nintr_pts; ++pt) {
      double const wt = b.wt_intr(pt);
      U = interp_vec_intr<NEQ>(soln, b, cell, pt, mask);
  ...
  simd_for_each<double>(execution::par, cell_grid, f);
...
void compute_side_integral(Block& block, int axis) {
  ...
  Basis const b = block.basis();
  int const nside_pts = num_pts(block.dim()-1, b.p);
  simd_view<double***> F = block.simd_flux(axis);
  simd_view<double***> R = block.simd_resid();
  auto f = [=] P3A_DEVICE (
      vector3<int> const& cell_ijk,
      device_simd_mask<double> const& mask) {
    device_simd<double> F_eq, val;
    int const cell = cell_grid.index(cell_ijk);
    for (int dir = 0; dir < ndirs; ++dir) {
      vector3<int> const side_ijk = get_cells_adj_side(cell_ijk, axis, dir);
      int const side = side_grid.index(side_ijk);
      double const sgn = get_dir_sign(dir);
      for (int pt = 0; pt < nside_pts; ++pt) {
...
void compute_gravity_source(Block& block, int soln_idx, double g, int axis) {
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) {
    ...
    for (int pt = 0; pt < nintr_pts; ++pt) {
      ...
      double const m_a = interp_scalar_intr(U, b, cell, pt, MM + axis);
      for (int m = 0; m < b.nmodes; ++m) {
...
void advance_explicitly(Block& block, int from_idx, int to_idx, double dt) {
  ...
  double const detJ = block.cell_detJ();
  Basis const b = block.basis();
  simd_view<double***> from = block.simd_soln(from_idx);
  simd_view<double***> to = block.simd_soln(to_idx);
  simd_view<double***> R = block.simd_resid();
  auto f = [=] P3A_DEVICE (
      vector3<int> const& cell_ijk,
      device_simd_mask<double> const& mask) {
    device_simd<double> from_eq, R_eq, val;
    int const cell = cell_grid.index(cell_ijk);
    for (int m = 0; m < b.nmodes; ++m) {
...
void reflect_boundary(Border& border) {
  ...
  int const dim = block.dim();
  grid3 const g = block.cell_grid();
  subgrid3 const bsides = generalize(get_adj_sides(g, axis, dir));
  grid3 const bside_grid(bsides.extents());
  int const nside_pts = num_pts(dim-1, b.p);
  View<double***> bsoln = border.soln(recv).val;
  auto f = [=] P3A_DEVICE (vector3<int> const& bside_ijk) {
    int const bside = bside_grid.index(bside_ijk);
    for (int pt = 0; pt < nside_pts; ++pt) {
      bsoln(bside, pt, MM + axis) *= -1.0;
    }
  };
  for_each(execution::par, bside_grid, f);
}
...
double compute_tally(Block& block, int eq) {
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) P3A_NEVER_INLINE {
    double tally = 0.;
    int const cell = cell_grid.index(cell_ijk);
    for (int pt = 0; pt < nintr_pts; ++pt) {
      double const wt = b.wt_intr(pt);
      double const U_eq = interp_scalar_intr(U, b, cell, pt, eq);
      tally += U_eq * wt * detJ;
    }
    return tally;
  };
...
double compute_L1_error(
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& cell_ijk) {
    ...
    for (int pt = 0; pt < nfine_intr_pts; ++pt) {
      ...
      double const uh = interp_scalar_fine(Uh, b, cell, pt, eq);
