...
DGT_METHOD constexpr int infer_dimension(Grid3 const& grid)
{
  return infer_dimension(grid.extents());
}

DGT_METHOD constexpr Grid3 dimensionalize(int const dim, Grid3 const& grid)
{
  Vec3<int> const extents = dimensionalize(dim, grid.extents());
  return Grid3(extents);
}

DGT_METHOD constexpr Grid3 generalize(int const dim, Grid3 const& grid)
{
  Vec3<int> const extents = generalize(dim, grid.extents());
  return Grid3(extents);
}
