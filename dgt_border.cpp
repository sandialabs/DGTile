...
static void verify(
    ...
    grid3 const& bside_grid,
    View<double***> U,
    static_array<View<double***>, ndirs> const& U_border,
    static_array<View<double**>, ndirs> const&  U_avg_border) {
  bool valid = true;
...
Message<View<double***>>& Border::soln(int msg_dir) {
  return m_soln[msg_dir];
}

Message<View<double**>>& Border::avg_soln(int msg_dir) {
...
static_array<View<double***>, ndirs> Border::soln() const {
  verify_border(*this);
  static_array<View<double***>, ndirs> r;
...
static_array<View<double****>, ndirs> Border::amr_soln() const {
  verify_border(*this);
  static_array<View<double****>, ndirs> r;
  int const left_send_or_recv  = (m_dir == left) ? recv : send;
  int const right_send_or_recv = (m_dir == left) ? send : recv;
...
void Border::allocate(int neq) {
  ...
  grid3 const g = m_node->block.cell_grid();
  subgrid3 const sides = generalize(get_adj_sides(g, m_axis, m_dir));
  int const nsides = sides.size();
  if (m_type == COARSE_TO_FINE) {
    resize(m_amr_flux, nsides, nchild, npts, neq);
  }
  for (int msg_dir = 0; msg_dir < ndirs; ++msg_dir) {
    if (m_type == COARSE_TO_FINE) {
      resize(m_amr[msg_dir].soln, nsides, nchild, npts, neq);
      resize(m_amr[msg_dir].avg_soln, nsides, nchild, neq);
      for (int which_child = 0; which_child < nchild; ++which_child) {
        resize(m_amr[msg_dir].child_soln[which_child].val, nsides, npts, neq);
        resize(m_amr[msg_dir].child_avg_soln[which_child].val, nsides, neq);
      }
    } else {
      resize(m_soln[msg_dir].val, nsides, npts, neq);
      resize(m_avg_soln[msg_dir].val, nsides, neq);
    }
  }
}
...
static static_array<View<double***>, ndirs> get_U(Border& border) {
  static_array<View<double***>, ndirs> r;
...
static static_array<View<double**>, ndirs> get_U_avg(Border& border) {
  static_array<View<double**>, ndirs> r;
...
static static_array<View<double****>, ndirs> get_amr_U(Border& border) {
  static_array<View<double****>, ndirs> r;
...
static static_array<View<double***>, ndirs> get_amr_U_avg(Border& border) {
  static_array<View<double***>, ndirs> r;
...
static void fill_border(Border& border, int soln_idx) {
  ...
  subgrid3 const sides = generalize(get_adj_sides(g, axis, dir));
  grid3 const border_side_grid(sides.extents());
  View<double***> U = block.soln(soln_idx);
  static_array<View<double***>, ndirs> U_border = get_U(border);
  static_array<View<double**>, ndirs> U_avg_border = get_U_avg(border);
  verify(dim, p, tensor, neq, cell_grid, border_side_grid, U, U_border, U_avg_border);
  auto f = [=] P3A_DEVICE (vector3<int> const& side_ijk) {
    vector3<int> const border_side_ijk = get_border_ijk(side_ijk, axis);
    vector3<int> const cell_ijk = get_sides_adj_cell(side_ijk, axis, idir);
  ...
  for_each(execution::par, sides, f);
}
...
static void fill_amr_border(Border& border, int soln_idx) {
  ...
  grid3 const border_side_grid(sides.extents());
  View<double***> U = block.soln(soln_idx);
  static_array<View<double****>, ndirs> U_border = get_amr_U(border);
  static_array<View<double***>, ndirs> U_avg_border = get_amr_U_avg(border);
...
static void fill_amr_buffer_from_border(
  ...
  subgrid3 const sides = generalize(get_adj_sides(g, axis, dir));
  grid3 const border_side_grid(sides.extents());
  vector3<int> const border_local = get_local(axis, border_which_child);
  ...
  auto f = [=] P3A_DEVICE (vector3<int> const& ijk) {
    vector3<int> const fine_ijk = map_to_fine(ijk, border_local, border_side_grid);
    vector3<int> const coarse_ijk = get_coarse_ijk(fine_ijk);
    vector3<int> const local = get_local_from_fine_ijk(fine_ijk);
  ...
  for_each(execution::par, border_side_grid, f);
}
...
void post_transfer(
    ...
    int data,
    int which_child,
    Message<View<T>>& send_msg,
    Message<View<T>>& recv_msg) {
  CALI_CXX_MARK_FUNCTION;
  Block const& block = border.node()->block;
...
static void post_transfer(Border& border) {
  ...
  if (type == BOUNDARY) {
    ...
    Block const& adj = border.adj()->block;
    int const axis = border.axis();
    vector3<int> const ijk = border.node()->pt().ijk;
    vector3<int> const local = get_local_from_fine_ijk(ijk);
    int const which_child = get_which_child(axis, local);
